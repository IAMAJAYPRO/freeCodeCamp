...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round((num_min_workers["salary"] == ">50K").mean() * 100,
                            1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country_vals = df[
        df["salary"] == ">50K"]["native-country"].value_counts(
        ) * 100 / df["native-country"].value_counts()
    highest_earning_country = highest_earning_country_vals.idxmax()
    highest_earning_country_percentage = round(
        highest_earning_country_vals.loc[highest_earning_country], 1)
